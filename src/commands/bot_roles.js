...
export const execute = async (interaction) => {
  ...
  if (!isAdmin(interaction)) {
    try { await interaction.followUp({ content: 'Nur Admins dürfen diese Aktion ausführen.' }); } catch {}
    return;
  }
  ...
  try {
    if (sub === 'add') {
      ...
      await interaction.followUp({ content: `Rolle <@&${role.id}> hinzugefügt. Aktuell erlaubt: ${after.map(id => `<@&${id}>`).join(', ') || '—'}` });
    } else if (sub === 'remove') {
      const role = interaction.options.getRole('role');
      const after = removeAllowed(role.id);
      await interaction.followUp({ content: `Rolle <@&${role.id}> entfernt. Aktuell erlaubt: ${after.map(id => `<@&${id}>`).join(', ') || '—'}` });
    } else if (sub === 'list') {
      const list = listAllowed();
      await interaction.followUp({ content: `Erlaubte Rollen: ${list.map(id => `<@&${id}>`).join(', ') || '—'}` });
    } else if (sub === 'reset') {
      resetAllowed();
      await interaction.followUp({ content: 'Alle erlaubten Rollen wurden gelöscht. Nur Admins dürfen Befehle nutzen.' });
    } else {
      await interaction.followUp({ content: 'Unbekannter Unterbefehl.' });
    }
  } catch (e) {
    await interaction.followUp({ content: `Fehler: ${e?.message || e}` });
  }
};
